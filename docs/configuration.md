...
The PostgreSQL adapter enumerates non-system base tables and captures rows as JSON. Tables with primary keys use key values for stable object IDs; tables without primary keys use a row-content digest. Redis keys and values are captured byte-for-byte with native DUMP/RESTORE payloads. Key names are Base64-encoded in snapshots so binary keys remain portable, and TTLs are restored and verified with a small elapsed-time allowance.
